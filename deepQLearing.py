...
class DeepQNetwork(object):
    def __init__(self, lr, n_actions, name, fc1_dims= 512,
                 input_dims= 17, chkpt_dir='tmp/dqn'):
        self.lr = lr
    ...
    def build_network(self):
        with tf.variable_scope(self.name):
            ...
            dense1 = tf.layers.dense(flatten, units=self.fc1_dims,
...
class Agent(object):
    def __init__(self, alpha, gamma, mem_size, n_actions, epsilon, batch_size,
                 replace_target=10000, input_dims=24,
    ...
    def learn(self):
        ...
        action_values = np.arange(40, dtype=np.int8)
        ...
        q_target[idx, action_indices] = q_target[idx, action_indices] + 0.8*(reward_batch + self.gamma *np.max(q_next, axis=1)*terminal_batch-q_target[idx, action_indices])
        ...
        if self.mem_cntr > 2500:#200000:
            if self.epsilon > 0.02:
                self.epsilon -= 1e-5
            elif self.epsilon <= 0.02:
                self.epsilon = 0.02
